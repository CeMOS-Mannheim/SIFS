...
#' @param point.size a numeric, the size of the points when `plotPoints=TRUE`.
...
plotOptical4 <- function(msiTeachingImgPath,
                         targetImgPath,
                         regions,
                         mpm,
                         transMat,
  ...
  # shad, updated it as of 02.05.2024.
  ...
  spp <- .transformspp(spp, transFun = misRegion$msiToOpticalTransFun)
  #sppimg <- .transformImg(sppimg, misRegion$msiToOpticalTransFun)

  tiff(file.path(outputPath, paste0(figLabel, "-msiTeachingImg.tiff")), width = 15, height = 15, res = 300,
       compression = "lzw", units = "cm")
  plot(msiTeachingImg, xlim = opticalXlim, ylim = rev(opticalYlim))

  moleculaR::plotAnalyte(spp, colourPal = colPal, add = T,
                         transpFactor  = alpha, pch = pch, size = point.size)
  ...
  #sppimg <- .transformImg(sppimg, .transmat2fun, shift = TRUE, transMat = transMat)
  spp <- .transformspp(spp, .transmat2fun, transMat = transMat)


  tiff(file.path(outputPath, paste0(figLabel, "-targetImg.tiff")), width = 15, height = 15, res = 300,
       compression = "lzw", units = "cm")
...
.transformImg <- function(img, transFun, shift = FALSE, ...){

  if(shift){
    xyRange <- data.frame(x = img$xrange - 1, y = img$yrange - 1)
    xCoords <- data.frame(x = img$xcol - 1, y = rep(1, length(img$xcol)))
    yCoords <- data.frame(x = rep(1, length(img$yrow)), y = img$yrow - 1)
  } else {
    xyRange <- data.frame(x = img$xrange, y = img$yrange)
    xCoords <- data.frame(x = img$xcol, y = rep(1, length(img$xcol)))
    yCoords <- data.frame(x = rep(1, length(img$yrow)), y = img$yrow)
  }
  ...
  xstep <- diff(xyRange$x)/length(xCoords$x)
  ystep <- diff(xyRange$y)/length(yCoords$y)
...
.transmat2fun <- function(coords, transMat){
  ...
  newCoords        <- floor(transMat %*% coords)
