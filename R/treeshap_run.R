...
t3_spmat_pos <- readRDS( file.path(abs_path, "data/input/01_spmatObjects/trainset_tissue/tr_spmat_XIII_t3_83318.rds"))
t4_spmat_pos <- readRDS( file.path(abs_path, "data/input/01_spmatObjects/testset_tissue/tst_spmat_XV_t1_82558.rds"))

lbls_pos_All <- readRDS( file.path(abs_path, "data/input/04_y_lbls/spectraLblsAll/04_Lbl_df_giant_pos/Lbls_df_Pos_all.rds"))
...
cat("Creating DMatrix with feature names...\n")
dtrain_named <- xgb.DMatrix(data = X_train, label = y_train, feature_names = MZ_cols)

xgb_annotated <- xgb.train( 
  params = list(
...
cat("Unifying model...\n")
unified_model <- unify(xgb_annotated, train_data_df)
